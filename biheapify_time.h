...
template<class T>
std::chrono::nanoseconds TimeBiHeapifyGivenVec(T &vec, const T &vec_original,
...
template<class T>
std::chrono::nanoseconds TimeBiHeapifySinglePassGivenVec(T &vec,
...
template<class T>
std::chrono::nanoseconds::rep TimeBiHeapifySinglePassOnGivenSize(
                           std::size_t total_num_nodes, int num_vecs_to_try = 1,
                           int num_repititions_per_vec = 1) {
  std::chrono::nanoseconds total{0};
  std::vector<T> vec(total_num_nodes), vec_original(total_num_nodes);
  for (int i = 0; i < num_vecs_to_try; i++) {
    randomhelpers::FillWithRandomNumbers(vec.begin(), vec.end(),
                  std::numeric_limits<T>::min(), std::numeric_limits<T>::max());
    vec_original = vec;
    total += TimeBiHeapifySinglePassGivenVec(vec, vec_original, total_num_nodes,
                                             num_repititions_per_vec);
  }
  return total.count();
}
