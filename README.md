# BiHeap
A BiHeap is a new independently developed data structure a part of whose data is lies a min heap, a part that lies in a max heap, and a part that lies simultaneously in both. An exposition on BiHeaps, including their definition, can be found in "BiHeaps and Pivot Selection.pdf". We use BiHeaps to define a double ended queue, called a BiQueue, that has amortized O(log N) insertions and pops (of the min and max elements). The minimum and maximum elements in a BiHeap are readily accessible. This project, among other things, implements the BiHeapify() function which given a random access iterator and a length, forms a BiHeap. 
